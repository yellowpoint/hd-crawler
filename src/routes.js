...
export const filterValidWikiLinks = (pageLinks) => {
    const validLinks = pageLinks.filter(link => {
        ...
        const actionRegex = /action=(history|edit)|title=|Help:|Special:|Wikipedia:|Main_Page|Candle#|ISBN|Doi/;
...
const getAhref = async (page) => {
    const pageLinks = await page.$$eval(".mw-body-content a:not(.reflist a):not(.navbox a)", (as) =>
        as
