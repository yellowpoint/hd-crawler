// https://github.com/BuilderIO/gpt-crawler/tree/main/src
...
// 亚马逊上是/tmp/datasets/default/
export const TempDir = isDev ? './storage' : '/tmp';
...
export function getPageHtmlWiki(page, selector = 'body') {
  return page.evaluate((selector) => {
...
export async function write(config) {
  ...
  const writeBatchToFile = async () => {
    ...
    console.log(
      `Wrote ${currentResults.length} items to ${path.resolve(nextFileNameString)}`,
    );
    currentResults = [];
    currentSize = 0;
    fileCounter++;
